...
class OpenInclude(sublime_plugin.TextCommand):
  ...
  def resolve_path(self, window, view, paths):
    ...
    for path in paths:
      ...
      extensions_or_patterns = ["", re.compile("^(a|b)/"), ".coffee", ".hbs", ".jade", ".js", ".scss", ".sass", ".styl", ".less"];
      for extension_or_pattern in extensions_or_patterns:
        # remove quotes
        ...
        if type(extension_or_pattern) is re._pattern_type:
          newpath = extension_or_pattern.sub('', path)
        else:
          newpath = path + extension_or_pattern
